...
public class ProgramadorTareasUI extends CustomComponent implements ClickListener, ValueChangeListener{
	...
	Logger log = Logger.getLogger(ProgramadorTareasUI.class.getName());
	...
	private void valueChangeJob(boolean esModoNuevoJob){
		if(esModoNuevoJob){
			...
			try {
				...
				log.error("+++ ERROR OBTENIENDO JOBDETAIL: " + e.getMessage());
				e.printStackTrace();
			}
...
//			listaService.actualizar(lista);
		...
		if(!validaDatosObligatoriosJob()){
			if(validaCreacionJob()){
				...
				try {
					...
					log.error("+++ ERROR AGREGANDO JOB: " + e.getMessage());
	...
	private void grabarTrigger(){
		try {
			...
			if(!validaDatosObligatoriosTrigger()){
				if(validaCreacionTrigger()){
		    		...
		    		if(flagNuevoTrigger){
		    			...
		    			log.info("+++ SE REGISTRO CORRECTAMENTE EL TRIGGER");
		    		}else{
		    			Item item = tblTrigger.getItem(tblTrigger.getValue());
		    			...
		    			log.info("+++ SE ACTUALIZO CORRECTAMENTE EL TRIGGER");
		    		}
		    		cargarTablaTrigger(jobDetail, true, null, null);			
		    		limpiar(TRIGGER);
				}else{
			...
			log.error("+++ OCURRIO UN ERROR AL CREAR TRIGGER: " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void buscarFiltrosTrigger(){
		...
		try {
			...
			log.error("+++ OCURRIO UN ERROR AL OBTENER JOBDETAIL " + e.getMessage());
			e.printStackTrace();
		}
	...
	private void eliminaJob(String grupoJob, String nombreJob){
		try {
			...
			log.error("+++ OCURRIO UN ERROR AL ELIMINAR EL JOB " + e.getMessage());
			e.printStackTrace();
		}
		cargarTablaJob(null);
	...
	private void eliminaTrigger(String grupoTrigger, String nombreTrigger){
		...
		try {
			...
			log.error("+++ OCURRIO UN ERROR AL ELIMINAR TRIGGER " + e.getMessage());
			e.printStackTrace();
		}
		cargarTablaTrigger(jobDetail, true, null, null);
	}
	...
	private void ejecutarTrigger(){
		try {
    		...
    		if(trigger!=null){
    		    ...
    		    log.info("+++ TRIGGER FUE EJECUTADO CORRECTAMENTE");	
    			...
    			log.error("+++ TRIGGER NO PUEDE SER EJECUTADO");	
    		}
		} catch (SchedulerException e) {
			log.error("+++ OCURRIO UN ERROR AL EJECUTAR TRIGGER " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void ejecutarJob(){
		try {
    		...
    		if(jobDetail!=null){
    		    ...
    		    log.info("+++ JOB FUE EJECUTADO CORRECTAMENTE");
    		    limpiar(JOB);
    		}else{
    			log.error("+++ JOB NO PUEDE SER EJECUTADO");	
    		}
		} catch (SchedulerException e) {
			log.error("+++ OCURRIO UN ERROR AL EJECUTAR JOB " + e.getMessage());
...
//			if(scheduler.isShutdown()){
			...
			log.error("+++ OCURRIO UN ERROR VALIDANDO SCHEDULER " + e.getMessage());
			e.printStackTrace();
		}
	}
...
//			if(scheduler.isShutdown()){
			if(scheduler.isInStandbyMode()){
				...
				log.error("+++ SCHEDULER SE ENCUENTRA ACTIVO");
			}
		} catch (SchedulerException e) {
			log.error("+++ OCURRIO UN ERROR AL INICIAR SCHEDULER " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void detenerScheduler(){
		try {
			if(scheduler.isStarted()){
				...
				log.error("+++ SCHEDULER SE ENCUENTRA DETENIDO");
			}
		} catch (SchedulerException e) {
			log.error("+++ OCURRIO UN ERROR AL DETENER SCHEDULER " + e.getMessage());
			e.printStackTrace();
		}
